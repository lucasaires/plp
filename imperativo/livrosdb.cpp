...
int consultaLivro(vector<string> &livros, int id)
{
    ...
    while(true) {
        ...
        const char *idLivros = reinterpret_cast<const char *>(sqlite3_column_text(stmt, U_ID));
        const char *nomeLivros = reinterpret_cast<const char *>(sqlite3_column_text(stmt, U_NOME));
        const char *escritorLivros = reinterpret_cast<const char *>(sqlite3_column_text(stmt, U_ESCRITOR));
        const char *paginasLivros = reinterpret_cast<const char *>(sqlite3_column_text(stmt, U_PAGINAS));
        const char *qnt_leitoresLivros = reinterpret_cast<const char *>(sqlite3_column_text(stmt, U_QNT_LEITORES));
        const char *notaLivros = reinterpret_cast<const char *>(sqlite3_column_text(stmt, U_NOTA));
        const char *generoLivros = reinterpret_cast<const char *>(sqlite3_column_text(stmt, U_GENERO));
...
int insereLivro(int id, string nome, string escritor, int paginas, int genero)
{
    ...
    string sql = "INSERT INTO livros(id, nome, escritor, paginas, 0, 0.0, genero) "
